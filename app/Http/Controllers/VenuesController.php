...
class VenuesController extends Controller
{
    ...
    public function show(string $type, Venue $venue, VenueAboutDataBuilder $aboutBuilder)
    {
        $user = request()->user();
        $venue = Venue::query()
            ->visibleFor($user)
        ...
        $about = $aboutBuilder->build($venue, $type);
    ...
    public function bookings(Request $request, string $type, Venue $venue)
    {
        ...
        $allowedStatuses = [
            ...
            EventBookingStatus::Paid->value,
            EventBookingStatus::Approved->value,
            EventBookingStatus::Cancelled->value,
        ];
    ...
    public function markPaidBooking(Request $request, string $type, Venue $venue, EventBooking $booking)
    {
        ...
        $bookingMode = $venue->settings?->booking_mode ?? VenueBookingMode::Instant;
        $nextStatus = $bookingMode === VenueBookingMode::Instant
        ...
        $notice = $nextStatus === EventBookingStatus::Approved
            ...
            : 'Бронирование помечено как оплаченное.';
