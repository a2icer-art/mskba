...
# full_description
Удалить поля `name` и `email` из `users`. Добавить `UserEmail` (1:N) с хранением подтверждения (confirmed_at). Сделать email необязательным при регистрации и обновить UI регистрации (обязательные поля отмечены *). Обновить отображение аккаунта и связанные процессы (миграции, сидеры, фабрики, shared props). Зафиксировать результат в проектной документации.
