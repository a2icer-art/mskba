...
1. Нажатие «Подтвердить» создаёт запись подтверждения:
   - для email/телефона генерируется одноразовый код;
   - для Telegram выдаётся deep-link на бота с одноразовым токеном.
...
6. Для Telegram подтверждение выполняется через бот (deep-link → кнопка «Подтвердить» в чате).

### Технические детали
- Генерация и проверка кода выполняется на бэкенде.
- Отправка кода реализована через централизованный сервис доставки; для email используется SMTP из /admin/settings, остальные типы пока заглушки.
...
4. Запрос на супервайзера может подать подтвержденный пользователь без активного контракта на площадке (кроме контракта создателя), если нет активного супервайзера.
5. Доступ к разделу супервайзинга и его настройкам определяется наличием активного контракта супервайзера (или ролью admin) и правами `venue.supervisor.view/manage`.
6. Для заявок на контракты в статусе `approved` права могут быть обновлены в модерации, изменения синхронизируются в контракте.
...
Подробности по методам оплаты и источникам реквизитов: `docs/project/venue_payment_methods.md`.

## Оплата события организатором (проектирование)
- Описание флоу оплаты и подтверждения: `docs/project/event_organizer_payment_flow.md`.
- Оплата становится доступна организатору только после перевода заявки в `awaiting_payment` администратором площадки.
- Подтверждение оплаты выполняется на странице бронирований площадки; при режиме `instant` бронирование переводится в `approved`.
