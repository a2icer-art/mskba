...
- Для подсказок используем директиву PrimeVue `v-tooltip`.
- Общие параметры и класс берём из `resources/js/Composables/useTooltip.js`.
- Для мобильных подсказок на не-фокусируемых элементах добавляем `tabindex="0"` и используем модификатор `.focus`.
