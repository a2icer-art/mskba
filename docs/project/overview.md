...
- Добавлен доменный модуль интеграций и подсказки адреса через Yandex (suggest + geocode) с фильтрацией по стране (ADDRESS_DEFAULT_COUNTRY) и городам из таблицы `cities`; при выборе адреса автоматически определяется ближайшая станция метро (по данным Yandex и локальному справочнику `metros`, с резервным поиском по координатам адреса).
